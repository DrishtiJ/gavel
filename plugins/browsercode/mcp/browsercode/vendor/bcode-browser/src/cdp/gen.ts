...
function renderType(p: Prop | CdpType, currentDomain: string): string {
  ...
  switch (p.type) {
    ...
    case 'array': {
      const inner = p.items
        ? renderType(p.items as Prop, currentDomain)
        : 'unknown'
      return `${inner}[]`
    }
    case 'object': {
...
async function build() {
    ...
    for (const c of realCmds) {
      ...
      const retType =
        c.returns && c.returns.length > 0
          ...
          : 'void'
      out.push(
        `${cdoc}    ${escId(c.name)}: ${paramSig} => Promise<${retType}>;`,
      )
    }
    out.push(`  };`)
  }
  ...
  for (const d of domains) {
    ...
    for (const c of realCmds) {
      ...
      out.push(
        `      ${escId(c.name)}: (params?: any) => t._call(${JSON.stringify(fq)}, params) as any,`,
      )
    }
    out.push(`    },`)
  }
  ...
  const totalCmds = domains.reduce(
    (n, d) => n + (d.commands?.filter((c) => !c.redirect).length ?? 0),
    0,
  )
  const totalTypes = domains.reduce((n, d) => n + (d.types?.length ?? 0), 0)
  out.push(
    `// Stats: ${domains.length} domains, ${totalCmds} commands (excl. redirects), ${totalTypes} types\n`,
  )
  ...
  console.log(
    `Domains: ${domains.length} | Commands: ${totalCmds} | Types: ${totalTypes}`,
  )
}
