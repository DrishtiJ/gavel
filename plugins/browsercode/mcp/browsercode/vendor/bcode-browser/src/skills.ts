...
const isCompiled =
  __dirname.replaceAll('\\', '/').match(/^\/\$bunfs\/|^B:\/~BUN\//) !== null
...
export const resolveSkillsDir = (dataDir: string): Promise<string> => {
  ...
  fresh.catch(() => {
    if (cache.get(dataDir) === fresh) cache.delete(dataDir)
  })
  return fresh
}
...
const materialize = async (target: string): Promise<string> => {
  ...
  const embed = isCompiled
    ? await import('bcode-skills.gen.ts').catch(() => null)
    : null
  if (isCompiled && !embed)
    throw new Error(
      'bcode-skills.gen.ts not found — was the build script updated?',
    )
  const want = `${embed?.buildHash ?? 'dev'}:${target}`
  if (
    embed &&
    (await Bun.file(path.join(target, SENTINEL))
      .text()
      .catch(() => null)) === want
  )
    return target
  ...
  await Promise.all(
    Object.entries(files).map(async ([rel, text]) => {
      ...
      await fs.writeFile(
        dest,
        text.replaceAll('{{SKILLS_DIR}}', target),
        'utf8',
      )
    }),
  )
...
const readEmbed = async (
  map: Record<string, string>,
): Promise<Record<string, string>> =>
  Object.fromEntries(
    await Promise.all(
      Object.entries(map).map(async ([rel, p]) => [
        rel,
...
const readDevSkills = async (): Promise<Record<string, string>> => {
  const rels = await Array.fromAsync(
    new Bun.Glob('**/*').scan({ cwd: DEV_SKILLS_DIR }),
  )
  return Object.fromEntries(
    await Promise.all(
      rels.map(async (rel: string) => [
        rel.replaceAll('\\', '/'),
