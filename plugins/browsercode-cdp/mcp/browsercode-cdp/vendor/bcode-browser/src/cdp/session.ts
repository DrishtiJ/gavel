...
export class Session implements Transport {
  ...
  > = []
  private callResultListeners: Array<
    (method: string, params: unknown, result: unknown) => void
  > = []
  ...
  async connect(opts: ConnectOptions = {}): Promise<void> {
    ...
    if (browsers.length === 0) {
      const scanned = getBrowserCandidates()
        .map((c) => c.name)
        .join(', ')
      throw new Error(
  ...
  private openWs(wsUrl: string, timeoutMs: number): Promise<void> {
    return new Promise<void>((res, rej) => {
      ...
      ws.addEventListener('error', (e) =>
        finish(
          new Error(
            `WS error: ${(e as any)?.message ?? 'connect failed (likely 403, permission not granted, or port closed)'}`,
          ),
        ),
      )
  ...
  async use(targetId: string): Promise<string> {
    const r = (await this._call('Target.attachToTarget', {
      targetId,
      flatten: true,
    })) as { sessionId: string }
  ...
  onEvent(
    fn: (method: string, params: unknown, sessionId?: string) => void,
  ): () => void {
   ...
   */
  onCallResult(
    fn: (method: string, params: unknown, result: unknown) => void,
  ): () => void {
    this.callResultListeners.push(fn)
    return () => {
      this.callResultListeners = this.callResultListeners.filter(
        (x) => x !== fn,
      )
    }
  }
  ...
  _call(method: string, params: unknown = {}): Promise<unknown> {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      return Promise.reject(
        new Error('Not connected. Call session.connect(...) first.'),
      )
    }
  ...
  private onMessage(raw: string): void {
    ...
    if (typeof m.id === 'number') {
      ...
      if (m.error)
        p.reject(new CdpError(m.error.code, m.error.message, m.error.data))
      else p.resolve(m.result)
    } else if (m.method) {
      for (const fn of this.eventListeners) {
        try {
          fn(m.method, m.params, m.sessionId)
        } catch {
          /* ignore */
        }
      }
...
export async function resolveWsUrl(
  ...
  throw new Error(
    'resolveWsUrl needs { wsUrl } or { profileDir }. For auto-detect, call session.connect() directly.',
  )
}
...
async function readDevToolsActivePort(
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (!Number.isFinite(port))
        throw new Error(`malformed port line: ${portStr}`)
      if (!path || !path.startsWith('/devtools/')) {
        // File is written atomically but path line may not be there on first open.
        throw new Error(
          `missing/invalid path line in DevToolsActivePort: ${JSON.stringify(text)}`,
        )
      }
      return { port, path }
    } catch (e) {
  ...
  throw new Error(
    `Polled ${filePath} for ${elapsed}ms (timeoutMs=${timeoutMs}): ${lastMessage}. ` +
...
function getBrowserCandidates(): BrowserCandidate[] {
  ...
  const push = (name: string, profileDir: string) =>
    list.push({ name, profileDir })
