...
function guessCategory(text) {
  ...
  if (/ps\d|xbox|nintendo|playstation|gaming|video game|console/.test(text))
    return 'video gaming'
  ...
  if (/car|truck|suv|van|honda|toyota|ford|bmw/.test(text))
    return 'cars & trucks'
  if (/motorcycle|harley|kawasaki|yamaha/.test(text))
    return 'motorcycles/scooters'
  if (/bicycle|bike/.test(text)) return 'bicycles'
  if (/guitar|piano|drum|keyboard|instrument/.test(text))
    return 'musical instruments'
  if (/camera|lens|tripod|dslr/.test(text)) return 'photo/video'
  if (/shirt|pants|shoes|jacket|dress|clothing/.test(text))
...
function mergeFields(session, extracted) {
  ...
  if (extracted.title && !session.data.title)
    session.data.title = extracted.title
  if (extracted.description && !session.data.description)
    session.data.description = extracted.description
  if (extracted.price && !session.data.price)
    session.data.price = extracted.price
  if (extracted.zipCode && !session.data.zip)
    session.data.zip = extracted.zipCode
}
...
function buildNextQuestion(session, prefix = '') {
  ...
  switch (session.state) {
    case 'ask_name':
      return `${pre}What's your first name?`
    case 'ask_item':
      return `${pre}What are you selling?`
    case 'ask_description':
      return `${pre}Can you describe it? (condition, age, brand, any flaws)`
    case 'ask_price':
      return `${pre}What price are you asking? (or "free")`
    case 'ask_zip':
      return `${pre}What's your ZIP code?`
    case 'ask_images':
...
async function handleMessage(conversationId, participant, text, mediaUrl) {
  ...
  if (
    /^(start over|restart|reset|hi|hello|hey)\.?$/i.test(t) &&
    session.state !== 'start'
  ) {
    session.state = 'start'
    ...
    await sendMessage(
      participant,
      "Hey! Welcome to Gavel 👋 Let's start fresh. What's your first name?",
    )
    await saveToConvex(conversationId, participant, session)
    return
  }
  ...
  switch (session.state) {
    case 'start':
      ...
      reply =
        "Hey! Welcome to Gavel 👋 I'm here to help you list your item on Craigslist. What's your first name?"
      break
    ...
    case 'ask_zip': {
      ...
      if (wasAsking === 'ask_description')
        session.data.description = extracted.description || t
      if (wasAsking === 'ask_price') session.data.price = extracted.price || t
      if (wasAsking === 'ask_zip') session.data.zip = extracted.zipCode || t
      if (wasAsking === 'ask_name')
        session.data.name = extracted.name || t.split(' ')[0]
    ...
    case 'ask_images':
      if (mediaUrl) {
        ...
        if (session.data.images.length === 0) {
          ...
          const suggested = guessCategory(
            (session.data.title || '') + ' ' + (session.data.description || ''),
          )
    ...
    case 'edit':
      if (/title|item/i.test(t)) {
        session.state = 'ask_item'
        session.data.title = null
        reply = 'What are you selling?'
      } else if (/desc/i.test(t)) {
        session.state = 'ask_description'
        session.data.description = null
        reply = 'Describe the item:'
      } else if (/price/i.test(t)) {
        session.state = 'ask_price'
        session.data.price = null
        reply = "What's the price?"
      } else if (/zip/i.test(t)) {
        session.state = 'ask_zip'
        session.data.zip = null
        reply = "What's your ZIP code?"
      } else if (/photo|image/i.test(t)) {
        session.data.images = []
        session.state = 'ask_images'
        reply = `Send your new photos, then type "done".`
      } else if (/category/i.test(t)) {
        session.state = 'confirm_category'
        reply = 'What category would you like?'
      } else {
        reply = `Which field? Reply: title, description, price, zip, photos, or category.`
      }
      break
    ...
    case 'done':
      ...
      reply =
        "Welcome back! 👋 Let's create a new listing. What's your first name?"
      break
...
async function initLastSeen() {
  try {
    const res = await fetch(
      'https://api.agentphone.ai/v1/conversations?limit=20',
      {
